...
async function createDropdownMenu(interaction, menu) {
    ...
    const embed = new EmbedBuilder()
        .setColor(0x0099FF)
        .setTitle(`📋 ${menu.name}`)
        .setDescription('利用したいWorkspaceを選択してください\n🔴満席 🟠残りわずか 🟢空きあり')
        .setTimestamp();
...
async function handleActivationCommand(interaction) {
    ...
    try {
        ...
        const email = interaction.options.getString('email');
        const workspace = getAllWorkspaces().find(w => w.id === ticket.workspaceId);
